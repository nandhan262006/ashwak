...
export default function EditServicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/services/${id}`)
      .then((r) => r.json())
      .then((item) => {
        if (item && !item.error) setForm({ title: item.title, subtitle: item.subtitle || "", description: item.description || "", image: item.image || "", sortOrder: item.sortOrder, isActive: item.isActive });
      })
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="mb-4 text-2xl font-bold text-white">Edit Service</h1>
      <Card>
